...
async function checkForUpdates() {
  const release = await window.azaan.checkForUpdates();
  if (!release) {
    return;
  }

  elements.updateMessage.textContent = `Version ${release.versionName} is ready to install.`;
  elements.updateButton.addEventListener("click", () => {
    window.azaan.openExternal(release.url);
  }, { once: true });
